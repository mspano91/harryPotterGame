...
CompraEscoba.addEventListener("click", () => {
  comprar(escoba);
});
CompraPocion.addEventListener("click", () => {
  comprar(pocion);
});
compraCapa.addEventListener("click", () => {
  comprar(capa);
});
CompraRana.addEventListener("click", () => {
  comprar(rana);
});
compraVarita.addEventListener("click", () => {
  comprar(varita);
});
compraMapa.addEventListener("click", () => {
  comprar(mapa);
});
...
function comprar(item) {
  if (oro - item.precio <= 0) {
    ...
    item.cantidad >= 4 ||
    mochila.length >= 4 ||
    item.cantidad + mochila.length === 6
  ) {
    esconderBagEmpty.innerText =
      "Harry your bag is fully...you might to sell some things";
  } else {
